...
# Weights for class sampling - bias toward confused pairs (AND/OR, XOR/NOR)
# Higher weight = more frequent sampling during training
CLASS_WEIGHTS = {
    "AND": 2.0,   # confused with OR
    "NAND": 1.0,
    "NOR": 2.0,   # confused with XOR
    "NOT": 1.0,
    "OR": 2.0,    # confused with AND
    "XNOR": 1.5,  # similar to XOR
    "XOR": 2.5,   # most problematic - confused with NOR
}
...
def _weighted_gate_choice(rng: random.Random) -> str:
    """Select a gate type using CLASS_WEIGHTS for biased sampling."""
    weights = [CLASS_WEIGHTS[name] for name in CLASS_NAMES]
    total = sum(weights)
    r = rng.random() * total
    cumulative = 0.0
...
def prepare_fixture_gate(gate_type: str, rng: random.Random) -> Image.Image:
    """
    Generate a fixture-style gate: plain text-labeled rectangle.

    Matches the exact style of the test fixtures - no visual markers,
    just text labels inside rectangles. Class bias in sampling handles
    the XOR/NOR confusion issue.
    """
    width = rng.randint(136, 144)
    height = rng.randint(98, 104)
    image = Image.new("RGBA", (width, height), (255, 255, 255, 0))
    draw = ImageDraw.Draw(image)

    # Simple rectangle with fixed light fill to match the shipped fixture schematics.
    fill_shade = 246
    outline_width = 2
    draw.rectangle(
    ...
    # Text label - primary identifier (matching fixture style)
    label = gate_type
    ...
    # Center text with slight random offset (matching fixture variation)
    text_x = (width - text_width) // 2 + rng.randint(-2, 2)
    text_y = (height - text_height) // 2 - 4 + rng.randint(-2, 2)
...
def place_gates(
    ...
    for column_index, count_here in enumerate(gates_per_column):
        ...
        for row_index in range(count_here):
            # Use weighted sampling to bias toward confused classes
            gate_type = _weighted_gate_choice(rng)
...
def draw_schematic(
    ...
    # Match the example fixtures: gate boxes are filled first and wires remain visible on top.
...
def generate_dataset(
    ...
    for split_name, count in split_counts.items():
        for split_index in range(count):
            ...
            image_extension = "png" if gate_style == "fixture" else "jpg"
            image_name = f"schematic_{image_index:05d}.{image_extension}"
            label_name = f"schematic_{image_index:05d}.txt"
            image_path = output_dir / "images" / split_name / image_name
            if gate_style == "fixture":
