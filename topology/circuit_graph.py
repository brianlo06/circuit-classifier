...
class CircuitGraph:
    ...
    def evaluate(self, input_values: Dict[str, bool]) -> Dict[str, bool]:
        ...
        observed_input_counts = self._observed_input_counts()
        ...
        for gate_id in self.get_topological_order():
            ...
            input_count = observed_input_counts.get(gate_id, GATE_INPUT_COUNTS.get(gate.gate_type.upper(), 2))
    ...
    def describe_outputs(self) -> Dict[str, str]:
        ...
        def expr_for_gate(gate_id: str) -> str:
            ...
            input_count = observed_input_counts.get(gate_id, GATE_INPUT_COUNTS.get(gate_type, 2))
            ...
            if gate_type == "NAND":
                return "NOT(" + " AND ".join(inputs) + ")"
            if gate_type == "OR":
                return "(" + " OR ".join(inputs) + ")"
            if gate_type == "NOR":
                return "NOT(" + " OR ".join(inputs) + ")"
            if gate_type == "XOR":
                return "(" + " XOR ".join(inputs) + ")"
            if gate_type == "XNOR":
                return "NOT(" + " XOR ".join(inputs) + ")"
            return gate_type
    ...
    def _evaluate_gate(gate_type: str, values: List[bool]) -> bool:
        ...
        if gate_type == "XOR":
            return sum(bool(value) for value in values) % 2 == 1
        if gate_type == "XNOR":
            return sum(bool(value) for value in values) % 2 == 0
        raise ValueError(f"Unsupported gate type '{gate_type}'")

    def _observed_input_counts(self) -> Dict[str, int]:
        counts = {gate_id: GATE_INPUT_COUNTS.get(node.gate_type.upper(), 2) for gate_id, node in self.gates.items()}
        for connection in self.connections:
            counts[connection.target_gate] = max(counts.get(connection.target_gate, 2), connection.target_input_index + 1)
        for primary_input in self.primary_inputs:
            for gate_id, input_index in primary_input.targets:
                counts[gate_id] = max(counts.get(gate_id, 2), input_index + 1)
        return counts
