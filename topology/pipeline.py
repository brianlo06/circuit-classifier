...
from .types import BoundingBox, GateDetection, GateReclassification, PipelineResult
...
class CircuitAnalysisPipeline:
    ...
    def analyze(
        ...
        return self._analyze_from_gates(
            image_path=Path(image_path),
            gates=list(detections) if detections is not None else None,
            image_input=image_input,
        )

    def analyze_symbol_style(
        self,
        image_path: Path,
        detections: Sequence[GateDetection],
        classifier_checkpoint: Optional[Path] = None,
        classifier_top_k: int = 3,
        suppress_edge_wires: bool = False,
        crop_padding_ratio: float = 0.12,
        gate_classifier: Optional[GateCropClassifier] = None,
        image_input: Optional[Image.Image] = None,
    ) -> PipelineResult:
        if not detections:
            raise ValueError("Symbol-style analysis requires at least one gate detection")

        reclassifications = self.reclassify_detections(
            image_path=Path(image_path),
        ...
        normalized_gates = self._normalize_symbol_topology_gates(
            self._apply_reclassifications(detections, reclassifications)
        )
        result = self._analyze_from_gates(
            image_path=Path(image_path),
        ...
        result.warnings.append(
            "Symbol-style analysis is a beta path that expects externally supplied detections in the real-symbol domain"
        )
        return result
    ...
    def reclassify_detections(
        ...
        classifier = gate_classifier or self._get_gate_classifier(
            checkpoint_path=Path(checkpoint_path) if checkpoint_path else DEFAULT_GATE_CLASSIFIER_CHECKPOINT,
            crop_padding_ratio=crop_padding_ratio,
        )
        return classifier.classify_detections(
            image_path=Path(image_path),
            detections=detections,
            top_k=top_k,
            suppress_edge_wires=suppress_edge_wires,
    ...
    def _get_gate_classifier(
        self,
        checkpoint_path: Path,
        crop_padding_ratio: float,
    ) -> GateCropClassifier:
        key = (str(Path(checkpoint_path)), float(crop_padding_ratio))
        classifier = self._gate_classifier_cache.get(key)
        if classifier is None:
            classifier = GateCropClassifier(
                checkpoint_path=Path(checkpoint_path),
                crop_padding_ratio=crop_padding_ratio,
            )
            self._gate_classifier_cache[key] = classifier
        return classifier

    def _analyze_from_gates(
        self,
        image_path: Path,
        gates: Optional[List[GateDetection]] = None,
        reclassifications: Optional[Sequence[GateReclassification]] = None,
        image_input: Optional[Image.Image] = None,
    ) -> PipelineResult:
        image_path = Path(image_path)
        ...
        warnings: List[str] = []
        gates = list(gates) if gates is not None else self.detect_gates(image_path)
        if not gates:
            warnings.append("No gates detected")

        wire_result = self.wire_detector.detect(image_source, gates)
        ...
        return PipelineResult(
            ...
            reclassifications=list(reclassifications or []),
    ...
    def _apply_reclassifications(
        detections: Sequence[GateDetection],
        reclassifications: Sequence[GateReclassification],
    ) -> List[GateDetection]:
        by_gate_id = {item.gate_id: item for item in reclassifications}
        normalized: List[GateDetection] = []
        for detection in detections:
            reclassified = by_gate_id.get(detection.gate_id)
            ...
            normalized.append(
                GateDetection(
                    ...
                    gate_type=reclassified.classifier_label.upper(),
                    bbox=detection.bbox,
                    confidence=reclassified.classifier_confidence,
                )
            )
        return normalized

    @staticmethod
    def _normalize_symbol_topology_gates(detections: Sequence[GateDetection]) -> List[GateDetection]:
        normalized: List[GateDetection] = []
        for detection in detections:
            bbox = detection.bbox
            gate_type = detection.gate_type.upper()
            aspect_ratio = bbox.width / max(bbox.height, 1.0)
            if gate_type in {"XOR", "XNOR"} and aspect_ratio >= 1.8 and bbox.y1 <= 8.0:
                bbox = BoundingBox(
                    x1=bbox.x1 + bbox.width * 0.12,
                    y1=bbox.y1,
                    x2=bbox.x2 - bbox.width * 0.22,
                    y2=bbox.y2 - bbox.height * 0.08,
                )
            elif gate_type in {"OR", "NOR"} and bbox.width >= 100 and bbox.height >= 90:
                bbox = BoundingBox(
                    x1=bbox.x1 + bbox.width * 0.18,
                    y1=bbox.y1 + bbox.height * 0.02,
                    x2=bbox.x2 - bbox.width * 0.30,
                    y2=bbox.y2 - bbox.height * 0.04,
                )
            normalized.append(
                GateDetection(
                    gate_id=detection.gate_id,
